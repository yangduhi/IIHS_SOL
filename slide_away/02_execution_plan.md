...
| 단계 | 이름 | 주요 목표 | 기본 출력 |
| --- | --- | --- | --- |
| 0 | 표준 동결 | 표준과 산출물 구조 고정 | 표준 sign-off |
| 1 | 케이스 마스터 | cohort 기준 테이블 구축 | `case_master.parquet` |
| 2 | 아웃컴마트 | PDF 중심 outcome mart 구축 | `outcomes_v1.parquet` |
| 3 | 피처마트 | barrier-relative feature mart 구축 | `features_v1.parquet` |
| 4 | 시간창 스윕 | 시간창 민감도 검증 | `window_sweep_summary.csv` |
| 5 | 모드 분석 | 2, 3, 4 mode 후보 비교 | `mode_study_summary.csv` |
| 6 | 결과 연계 | mode와 safety 결과 연결 | `ri_vs_safety_map.*` |
| 7 | 사례집 | 대표 사례 및 matched-pair 정리 | `matched_pair_casebook.md` |
| 8 | 최종 합성 | 결론, 제한점, 다음 단계 정리 | `final_study_brief.md` |
...
- 모드 분석에 바로 투입할 수 있는 feature mart가 생성된다.
