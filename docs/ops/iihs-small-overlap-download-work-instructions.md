...
This document is now an archive/reference guide for the acquisition phase.

For the current database-build phase, start with:

- `docs/README.md`
- `docs/core/database-build-playbook.md`
- `docs/core/research-database-architecture.md`

## Goal

Build a repeatable project that downloads IIHS TechData evaluation-result assets for:

- `Small overlap frontal: driver-side`
- `Small overlap frontal: passenger-side`

Download scope for both categories:

- Include evaluation-result assets exposed through folders such as `DATA*` and `REPORTS`.
- Exclude all video payloads, including the `VIDEO` folder and direct video files.
- Exclude all photo payloads, including the `PHOTOS` folder and direct image files.
...
- `VIDEO` and `PHOTOS` are visible in authenticated detail pages, but the final delivery scope for this project excludes both folders and their file payloads.
...
4. Record the confirmed site facts in `docs/ops/field-notes.md`.
...
- A short note in `docs/ops/field-notes.md` describing whether MFA, forced re-login, or consent prompts appear.
...
5. Record:
   ...
   - How `VIDEO` and `PHOTOS` entries are represented so they can be excluded deterministically
...
- One non-video sample file downloaded end-to-end and written to disk.
...
4. Exclude `VIDEO` and `PHOTOS` folders, plus video and image files, before scheduling downloads.
5. Download each in-scope file into a deterministic path.
6. Write file-level metadata and checksum.
7. Mark the file group complete only when every listed in-scope file is downloaded successfully.

Required exclusion rules:

- Skip any folder path that starts with `VIDEO`.
- Skip any folder path that starts with `PHOTOS`.
- Skip any normalized relative path containing `\VIDEO\` or `/VIDEO/`.
- Skip any normalized relative path containing `\PHOTOS\` or `/PHOTOS/`.
- Skip files with video extensions such as `.mp4`, `.mov`, `.avi`, `.wmv`, `.mkv`, and `.webm`.
- Skip image files such as `.jpg`, `.jpeg`, `.png`, `.tif`, `.tiff`, `.bmp`, and `.gif`.
- Do not use a folder-level bulk download action if it targets `VIDEO`.
- Do not use a folder-level bulk download action if it targets `PHOTOS`.
...
3. Compare in-scope folder/file counts against the live detail page for a random sample while ignoring `VIDEO` and `PHOTOS`.
4. Verify no downloaded path contains `VIDEO`.
5. Verify no downloaded path contains `PHOTOS`.
6. Verify no downloaded file has a video extension such as `.mp4`, `.mov`, `.avi`, `.wmv`, `.mkv`, or `.webm`.
7. Verify no downloaded file has an image extension such as `.jpg`, `.jpeg`, `.png`, `.tif`, `.tiff`, `.bmp`, or `.gif`.
8. Verify no zero-byte files remain.
9. Re-open a random completed file group and confirm manifest parity.
...
3. `scripts/download-filegroup.mjs`
   Downloads all in-scope non-video, non-photo files for one file group and writes manifest rows.
...
- One non-video, non-photo sample file can be downloaded and checksummed.
- `VIDEO` and `PHOTOS` folder entries are detected and excluded correctly.
...
- Example folder contents on that detail page:
  ...
  - `REPORTS`
  - `VIDEO`
- Delivery rule for authenticated detail pages:
  - enumerate `VIDEO` and `PHOTOS` only to confirm exclusion logic
  - do not download `VIDEO` files, video extensions, `PHOTOS` files, or image extensions
