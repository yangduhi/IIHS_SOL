...
- Authenticated browser-context `fetch()` to the same file URL returned:
  ...
  - size: `86333` bytes
- The project delivery scope is narrower than the visible detail page contents: `VIDEO` and `PHOTOS` must be excluded from download output even when present in the authenticated tree.
...
## Delivery Scope Override

- Keep only `Small overlap frontal: driver-side` and `Small overlap frontal: passenger-side`.
- Download evaluation-result assets only.
- Exclude the `VIDEO` folder, the `PHOTOS` folder, and direct video/image files from download, manifest completion, and validation.
